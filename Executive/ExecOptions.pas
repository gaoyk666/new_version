...
Begin
     ...
     OptionHelp[89] := 'Numeric marker code (0..47 see Users Manual) for PVSystems and PVSystem. Default is 15.';
     ...
     OptionHelp[93] := 'Size of PVsystem and PVSystem markers. Default is 1.';
     ...
     OptionHelp[113] := '(default -1)Gets/Sets the CPU to be used by the active actor. If negative (-1) means that the actor affinity is to all the CPUs and will be executed in the' +CRLF+
                        'first available CPU and will be realocated into another CPU dynamically if the operating system requires it. By setting a CPU number for an actor will force' +CRLF+
                        'the actor to be executed only on the specific CPU.';
     OptionHelp[114] := 'Gets progress (%) for all the actors when performing a task';
     OptionHelp[115] := 'Activates/Deactivates the parallel machine in OpenDSS, if deactivated OpenDSS will behave sequentially';
     ...
     OptionHelp[119] := 'Percentage of coverage expected when estimating the longest paths on the circuit for tearing, the default coverage'+CRLF+
                        'is the 90% (0.9), this value cannot exceed 1.0. When used with the "Set" command is used for the algorithm for estimating the paths within the circuit'+CRLF+
     ...
     OptionHelp[127] := '{YES/TRUE | NO/FALSE} Default = FALSE. Enables/disables the seasonal selection of the rating for determining if an element is overloaded. When enabled, the energy meter will' +
                        'look for the rating (NormAmps) using the SeasonSignal  to evaluate if the PDElement is overloaded';
     OptionHelp[128] := 'It is the name of the XY curve defining the ratings seasonal change for the PDElements in the model when performing QSTS simulations. The seasonal ratings need to be defined' +
  ...
  Try
     ...
     Begin
         ...
         CASE ParamPointer OF
          ...
          114: begin {$IFNDEF FPC} {$IFNDEF CONSOLE}
