               ...
               Begin
                  ...
                  Begin
                     ...
                     If Factor > ActiveCircuit[ActiveActor].NormalMinVolts Then
                        Result := Color1
                     Else If Factor > ActiveCircuit[ActiveActor].EmergMinVolts Then
                        Result := Color2
                     Else
                        Result := Color3;
                  End
 ...
 TRY
   Case PlotType of
      ptmonitorplot: Begin
         ...
         End;
       End;   {Monitor Plot}
      ptLoadShape: If MakeNewGraph(GetOutputDirectory + CircuitName_[ActiveActor] + Format('Loadshape_%s.DSV', [ObjectName])) > 0 Then
         Begin
         ...
         End Else Begin
            ...
            Exit;
         End;
      ptTShape: If MakeNewGraph(GetOutputDirectory + CircuitName_[ActiveActor] + Format('TempShape_%s.DSV', [ObjectName])) > 0 Then
         Begin
         ...
         End Else Begin
            ...
            Exit;
         End;
      ptPriceShape:If MakeNewGraph(GetOutputDirectory + CircuitName_[ActiveActor] + Format('Priceshape_%s.DSV', [ObjectName])) > 0 Then
         Begin
         ...
         End Else Begin
            ...
            Exit;
         End;
      ptProfile: If MakeNewGraph(GetOutputDirectory + CircuitName_[ActiveActor] + Format('Profile%d.DSV', [PhasesToPlot])) > 0 Then
         Begin
            DoProfilePlot;
            Exit;
         End Else Begin
   ...
   ELSE { All other plots }
      ...
      case PlotType of
          ptAutoAddLogPlot:If MakeNewGraph(GetOutputDirectory + CircuitName_[ActiveActor] + 'AutoADD.DSV') = 0 Then
                           Begin
          ...
          ptCircuitplot: Begin
                            ...
                            If MakeNewGraph(Fname) = 0 Then
                             ...
                             End;
                         End;
          ptGeneralDataPlot: If MakeNewGraph(GetOutputDirectory + CircuitName_[ActiveActor] + 'General.DSV') = 0 Then
                           Begin
                              DoSimpleMsg('Make New Plot failed for General Data Plot.', 87347);
                              Exit;
                           End;
          ptGeneralCircuitPlot: If MakeNewGraph(GetOutputDirectory + CircuitName_[ActiveActor] + 'GeneralCircuit.DSV') = 0 Then
                           Begin
                              DoSimpleMsg('Make New Plot failed for GeneralCircuit Plot.', 87348);
                              Exit;
                           End;
          ptMeterZones: If MakeNewGraph(GetOutputDirectory + CircuitName_[ActiveActor] + 'MeterZone.DSV') = 0 Then
                           Begin
                              DoSimpleMsg('Make New Plot failed for MeterZone Plot.', 87349);
                              Exit;
                           End;
          ptdaisyplot: If MakeNewGraph(GetOutputDirectory + CircuitName_[ActiveActor] + 'Daisy.DSV') = 0 Then
                           Begin
...
begin
     ...
     If ActiveCircuit[ActiveActor].MarkTransformers Then MarkTheTransformers;
...
begin
      ...
      Element.ComputeIterminal(ActiveActor); Element.ComputeVTerminal(ActiveActor);
