...
Begin
  ...
  GISHelp[15] :=
    ...
    + CRLF + CRLF + '- Streets' + CRLF + '- StreetsVector' + CRLF +
    '- StreetsNight' + CRLF + '- Satellite' + CRLF + '- SatelliteLabels' + CRLF
    + '- SatelliteLabelsVector' + CRLF + '- DarkGrayCanvas' + CRLF +
  ...
  GISHelp[22] :=
    ...
    + 'The coordiantes must be defined using GISCoords, the size and color can be specified through the options GISColor and GISThickness.'
    + CRLF + 'The shape can be one fo the following:' + CRLF + CRLF + '  Circle'
    + CRLF + '  + ' + CRLF + '  Diamond' + CRLF + '  Square' + CRLF +
  ...
  GISHelp[25] :=
    ...
    + 'the original format and the second argument the destination format. The format can be one of the following:'
  ...
  GISHelp[26] :=
    ...
    + ' containing the coordinates, which should be organized in 2 columns comma separated. The format can be one of the following:'
    + CRLF + CRLF + '- LatLong (latitude, Longitude - WGS84))' + CRLF +
    '- DMS (Degrees, minutes, seconds) ' + CRLF +
    '- UTM (Universal Transverse Mercator)' + CRLF + '- USNG' + CRLF + CRLF +
    'The following conditions need to be fulfilled:' + CRLF + CRLF +
...
Begin
  if IsGISON then
  Begin
    InMsg := '{"command":"route","coords":{"long1":' + floattostr(GISCoords^[1])
      + ',"lat1":' + floattostr(GISCoords^[2]) + ',"long2":' +
...
Begin
  if IsGISON then
  Begin
    JSONCmd := '{"command":"showroute"},"color":"' + GISColor + '","thickness":'
      + GISThickness + '}';
    try
      GISTCPClient.IOHandler.WriteLn(JSONCmd);
      InMsg := GISTCPClient.IOHandler.ReadLn(#10, 2000);
