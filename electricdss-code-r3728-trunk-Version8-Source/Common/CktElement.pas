...
// Check for an almost certain programming error
    ...
    If Value <> Fnconds Then ActiveCircuit[ActiveActor].BusNameRedefined := TRUE;
    Fnconds := Value;
...
VAR
   i    : Integer;
Begin
  For i := 1 to Fnphases DO cMBuffer^[i] := cabs(Iterminal^[i]);
End;
           ...
           FOR i := 1 to Fnconds DO     // 11-7-08 Changed from Fnphases - was not accounting for all conductors
             Begin
              ...
              if not ADiakoptics or (ActorID = 1) then
                ...
                IF  n > 0 THEN  Caccum(cPower, Cmul(VoltInActor1(n), conjg(Iterminal[k+i]) ))
       ...
       WITH ActiveCircuit[ActorID].Solution DO Begin
         ...
         IF  n > 0 THEN  Begin
          ...
          Begin
           ...
           IF    ActiveCircuit[ActorID].PositiveSequence
           THEN  Caccum(cLoss, CmulReal(Cmul(VoltInActor1(n), conjg(Iterminal^[k])), 3.0))
           ELSE  Caccum(cLoss, Cmul(VoltInActor1(n), conjg(Iterminal^[k])));
          End;
         End;
       End;
     End;
...
Begin
  ...
  case ValType of
    0,7 : Result := -Power[1, actorID].re;             // P, P0
    1,8 : Result := -Power[1, actorID].im;             // Q, Q0
    2 : Result := MaxVoltage[1, ActorID];             // VMag
    3 : Result := MaxVoltageAng[1, ActorID];          // VAng
    4 : Result := MaxCurrent[1, ActorID];             // IMag
    5 : Result := MaxCurrentAng[1, ActorID];          // IAng
    6 : Result := cabs(Power[1, actorID]);            // S
    else
      Result  := 0;
  end;
         ...
         Begin
            ...
            IF   n > 0 THEN Begin
              ...
              Begin
               ...
               IF   ActiveCircuit[ActorID].PositiveSequence
               THEN PowerBuffer^[i] := CmulReal(Cmul(VoltInActor1(n), conjg(Iterminal^[i])), 3.0)
               ELSE PowerBuffer^[i] := Cmul(VoltInActor1(n), conjg(Iterminal^[i]));
              End;
            END;
         End;
     End
