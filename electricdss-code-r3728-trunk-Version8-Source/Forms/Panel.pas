...
begin
  ...
  if Not ActiveCircuit[ActiveActor].IsSolved then ResultPages.ActivePage:=SummaryTab;
  Screen.Cursor := crDefault;
end;
...
// If a command line file name give, attempt to execute the script
  If CmdLineFileFound Then Begin
    DSSExecutive[ActiveActor].Command := 'compile ' + EncloseQuotes(CmdLineFileName);
...
begin
      ...
      Begin
        ...
        End;
        ActiveCircuit[ActiveActor].SetElementActive (Classbox.text + '.' + elementbox.text);
      End;
end;
...
begin
        ...
        Begin
           ...
           Edit1.text := Format('%-.6g',[ActiveCircuit[ActiveActor].loadmultiplier]);
           Showmodal;
...
begin
    If ActiveCircuit[ActiveActor]=nil then exit;  // do nothing
...
begin
        ...
        Begin
           ...
           Edit1.text := Format('%-g' ,[(ActiveCircuit[ActiveActor].DefaultGrowthRate-1.0)*100.0]);
           Showmodal;
           If OKPressed Then Begin
     ...
     If ActiveCircuit[ActiveActor]=nil Then Exit;
        ...
        Begin
            ...
            For i := 1 to ActiveCircuit[ActiveActor].NumBuses Do
              ComboBox1.Items.Add(ActiveCircuit[ActiveActor].BusList.Get(i));
...
begin
            If ActiveCircuit[ActiveActor] <> Nil Then ActiveScriptForm.ExecuteDSSCommand('MakebusList')
...
begin
  ...
  Caption := ProgramName + ' Data Directory: ' + DataDirectory[ActiveActor];
end;
...
begin
     With OpenDialog1 Do
      Begin
           ...
           Filter := 'any File|*.*';
           FileName := DataDirectory[ActiveActor]+'*.*';
...
begin
    If assigned(ActiveCircuit[ActiveActor]) Then  With PlotOptionsForm Do
