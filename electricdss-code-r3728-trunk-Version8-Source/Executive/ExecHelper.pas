...
Begin
     ...
     Dummy := AllocMem(Sizeof(Double) * 300); // Big Buffer
     Num   := Parser[ActiveActor].ParseAsVector(300, Dummy);
...
Begin
   Result := 0;
   {$IFDEF FPC}initialize(Dummy);{$ENDIF}
...
Begin
    ...
    CASE Param[1] of
     'A': Begin
              ...
              While metobj <> nil Do
              Begin
                if metobj.Enabled then MetObj.InterpolateCoordinates;
                MetObj := ActiveCircuit[ActiveActor].EnergyMeters.Next;
              End;
          End;
