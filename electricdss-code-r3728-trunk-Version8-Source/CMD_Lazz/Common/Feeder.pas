   ...
   TFeederObj = class(TPCElement)
     private
      ...
      public
        ...
        PROCEDURE InitializeFeeder(const BranchList:TCktTree);
...
USES  ParserDel, Circuit, DSSClassDefs, DSSGlobals, Sysutils, Command, Energymeter;
     ...
     If BranchList <> Nil Then Begin
       ...
       While pElement <> Nil Do Begin
           ...
           For i := 1 to pElement.NTerms-1 Do Begin
             ...
             If bref > 0 Then  ActiveCircuit.Buses^[bref].IsRadialBus := TRUE;
           End;
...
//----------------------------------------------------------------------------
Procedure TFeederObj.RecalcElementData;
...
//----------------------------------------------------------------------------
Procedure TFeederObj.CalcYPrim;
...
Function TFeederObj.InjCurrents:Integer;
...
Procedure TFeederObj.GetCurrents(Curr: pComplexArray);
...
(*   If ActiveCircuit.RadialSolution Then
   Begin
     TRY
...
procedure TFeederObj.MakePosSequence;
begin
 { Do Nothing
  If Fnphases>1 Then
  Begin
