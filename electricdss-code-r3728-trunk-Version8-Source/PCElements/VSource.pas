  ...
  WITH ActiveVSourceObj DO Begin
     ...
     WHILE Length(Param) > 0 DO Begin
         ...
         CASE ParamPointer OF
           ...
           28: DailyShape   := Param;
           29: DutyShape    := Param;
           30: IsQuasiIdeal := InterpretSourceModel(Param);
           31: puZideal  := InterpretComplex(Param);
         ELSE
...
Begin
     if CompareTextShortest(S, 'ideal')=0 then Result := TRUE
     Else Result := FALSE;
End;
...
begin
        Case Index of
          ...
          31 : Result := Format('[%-.8g, %-.8g]',[ puZideal.re , puZideal.im ]);
...
Begin
     IF DailyShapeObj <> Nil THEN
       ...
       End
     ELSE ShapeFactor := cmplx(PerUnit, 0.0);   // Default to no daily variation
End;
...
{Yearly curve is assumed to be hourly only}
     ...
     End
     ELSE ShapeFactor := cmplx(PerUnit, 0.0);   // Defaults to no variation
End;
