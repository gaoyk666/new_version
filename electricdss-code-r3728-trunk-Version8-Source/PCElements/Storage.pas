...
USES  StorageVars, StoreUserModel, DSSClass, PCClass, PCElement, ucmatrix, ucomplex,
      LoadShape, Spectrum, ArrayDef, Dynamics, XYCurve, InvDynamics, mathutil;
...
Begin
     ...
     Begin
         ...
         If      (Level > 0.0) and (kWhStored - kWhReserve > EPSILON) Then StorageState := STORE_DISCHARGING
         ELSE If (Level < 0.0) and (kWhStored - kWhRating < -EPSILON)  Then StorageState := STORE_CHARGING
...
Begin
    ...
    CASE i of
       ...
       2:   Begin
              ...
              Begin
                if CheckIFDelivering(ActiveActor) then  Result  :=  STORE_DISCHARGING
                else if kWhStored  =  kWhRating then Result :=  STORE_IDLING else Result :=  STORE_CHARGING;
              End;
            End;
       3,4: Begin
              ...
              A :=  A or B;
              if i = 4 then A :=  not A;
              If A Then Result := abs(Power[1,ActiveActor].re*0.001) else Result := 0.0;
            End;
