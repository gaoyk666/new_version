...
TYPE
   ...
   public
       ...
       PROCEDURE TakeSample(ActorID : Integer);  Virtual;    // Sample control quantities and set action times in Control Queue
       Procedure AllocateSensorArrays;
       Procedure CalcAllocationFactors(ActorID : Integer);
   end;
...
procedure TMeterElement.CalcAllocationFactors(ActorID : Integer);
Var
   iOffset   :Integer;
   i         :Integer;
   Mag       :Double;
...
Begin
    If assigned(SensorCurrent)       then Reallocmem(SensorCurrent, 0);
    If assigned(SensorVoltage)       then Reallocmem(SensorVoltage, 0);
    If assigned(CalculatedCurrent)   then Reallocmem(CalculatedCurrent, 0);
    If assigned(CalculatedVoltage)   then Reallocmem(CalculatedVoltage, 0);
    If assigned(PhsAllocationFactor) then Reallocmem(PhsAllocationFactor, 0);

   Inherited Destroy;
End;


procedure TMeterElement.TakeSample(ActorID : Integer);
begin
