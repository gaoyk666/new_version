...
Begin
     MaxBusNameLength := 4;
     With ActiveCircuit[ActiveActor] Do FOR i := 1 to NumBuses DO MaxBusNameLength := Max(MaxBusNameLength, Length(BusList.Get(i)));
End;
...
Begin
     With ActiveCircuit[ActiveActor] Do Begin
       ...
       For j := 1 to NumNodesThisBus DO Begin
         ...
         Volts := ActiveCircuit[ActiveActor].Solution.NodeV^[nref1];
         ...
         // Line-to-line voltages
           Begin         // Convert to Line-Line assuming no more than 3 phases
              ...
              IF kk <= NumNodesThisBus Then Begin
                  ...
                  VoltsLL := Csub(Volts, ActiveCircuit[ActiveActor].Solution.NodeV^[nref2]);
              End;
           End;
...
Begin
  ...
  For i := 1 to NCond Do Begin
       ...
       If Node1 > 0 then Bus1  := ActiveCircuit[ActiveActor].MapNodeToBus^[Node1].BusRef else Bus1 := 0;
       If Node2 > 0 then Bus2  := ActiveCircuit[ActiveActor].MapNodeToBus^[Node2].BusRef else Bus2 := 0;
...
Begin
  ...
  Try
   ...
   1: Begin
     ...
     FOR i := 1 to ActiveCircuit[ActiveActor].NumBuses DO WriteBusVoltages(F, i, LL);
...
BEGIN
  ...
  Try
    ...
    Begin
      Ctotal := CZERO;
      IF (CLASSMASK AND pElem.DSSObjType) =  AUTOTRANS_ELEMENT
      THEN Ntimes := pElem.Nphases    // Special case for AutoTrans
      ELSE Ntimes := NCond;
...
Begin
  ...
  Try
   ...
   0:Begin
     ...
     Begin
       ...
       THEN Begin
        NCond := pDElem.NConds;
        Nterm := pDElem.Nterms;
        PDElem.GetCurrents(c_Buffer, ActiveActor);
   ...
   1: Begin
     ...
     // PDELEMENTS first
     p_Elem := ActiveCircuit[ActiveActor].PDElements.First;
     ...
     WHILE p_Elem<>nil DO Begin
       IF p_Elem.Enabled THEN Begin
        ...
        IF (CLASSMASK AND P_Elem.DSSObjType) =  AUTOTRANS_ELEMENT
        THEN Ntimes := P_Elem.Nphases    // Special case for AutoTrans
        ELSE Ntimes := NCond;
        ...
        FOR j := 1 to NTerm Do Begin
          ...
          Writeln(F, '   ', Cabs(Saccum)/1000.0:8:1,'     ', PowerFactor(Saccum):8:4);
          FromBus := Pad(StripExtension(p_Elem.Nextbus),MaxBusNameLength);
          IF (CLASSMASK AND P_Elem.DSSObjType) =  AUTOTRANS_ELEMENT   Then Inc(k, Ntimes);
        End;
        Writeln(F);
       End;
  ...
  Try
   ...
   1: Begin
     ...
     End;
      ...
      // FAULTs next
     p_Elem := ActiveCircuit[ActiveActor].Faults.First;
     ...
     WHILE p_Elem<>nil DO Begin
      ...
      p_Elem := ActiveCircuit[ActiveActor].Faults.Next;
     End;
...
{$ENDIF}
   ...
   { Solve for Fault Injection Currents}
           ...
           With Buses^[iBus] Do Begin
             ...
             For iphs := 1 to NumNodesThisBus Do Begin
               For iphs2 :=1 to NumNodesThisBus do Begin

                if iphs >= iphs2 then continue;
...
Begin
  ...
  Try
   ...
   ELSE Begin  // Default - Just do PD and PC Element in active circuit
     ...
     Writeln(F);
     Writeln(F,'Elements in Active Circuit: ' + ActiveCircuit[ActiveActor].Name);
     Writeln(F);
     Writeln(F, 'Power Delivery Elements');
     Writeln(F);
     ...
     Writeln(Fdisabled);
     Writeln(Fdisabled,'DISABLED Elements in Active Circuit: ' + ActiveCircuit[ActiveActor].Name);
     Writeln(Fdisabled);
...
Begin
  ...
  Try
     ...
     Writeln(F);
     Writeln(F,'BUSES AND NODES IN ACTIVE CIRCUIT: ' + ActiveCircuit[ActiveActor].name);
     Writeln(F);
...
Begin
  ...
  Try
     ...
     End Else Begin;
         ...
         pElem := ActiveCircuit[ActiveActor].energyMeters.First;   // write registernames for first meter only
...
Begin
  ...
  Try
     ...
     PCelem := ActiveCircuit[ActiveActor].Loads.First;
     WHILE Pcelem<>nil DO
     Begin
         If PcElem.Enabled Then Begin
             Caccum(LoadPower, PCelem.Power[1,ActiveActor]);
         End;
         PCelem := ActiveCircuit[ActiveActor].Loads.Next;
     End;
...
Begin
    ...
    // Get Started at main voltage source
       TestElement := ActiveCircuit[ActiveActor].Sources.First;
...
Begin
  Try
    ...
    ReWrite(Ftree);
    Writeln(Ftree,'Branches and Loads in Circuit ' + ActiveCircuit[ActiveActor].Name);
    Writeln(Ftree);
        ...
        // Now Sum in each device current, keep track of the largest current at a node.
           ...
           Begin
              ...
              Begin
                 ...
                 IF (CLASSMASK AND pCktElement.DSSObjType) =  AUTOTRANS_ELEMENT   Then Begin
...
Begin
      ...
      Try
        ...
        {Check Loads}
        If ActiveCircuit[ActiveActor].Loads.ListSize >0 then  Begin
            Writeln(F);
        ...
        while pLoad <> Nil do  Begin
           {Find Bus To Which Load Connected}
            pBus := ActiveCircuit[ActiveActor].Buses^[pLoad.Terminals^[1].BusRef];
            BusName := ActiveCircuit[ActiveActor].BusList.Get(pLoad.Terminals^[1].BusRef);
        ...
        If ActiveCircuit[ActiveActor].Generators.ListSize >0 then  Begin
            Writeln(F);
        ...
        while pGen <> Nil do  Begin
           {Find Bus To Which Generator Connected}
            pBus := ActiveCircuit[ActiveActor].Buses^[pGen.Terminals^[1].BusRef];
            BusName := ActiveCircuit[ActiveActor].BusList.Get(pGen.Terminals^[1].BusRef);
            If pBus.kVBase <> 0.0 then  Begin
